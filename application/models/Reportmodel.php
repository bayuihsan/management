...
// db2 digunakan untuk mengakses database ke-2
	...
	public function getMaxDate(){
		//get Max Date
		$max_query=$this->db2->query("SELECT max(DATE_FORMAT(tanggal_aktif,'%Y-%m-%d')) as tgl_max FROM new_psb 
		WHERE status='sukses'");	
	...
	public function getCurMont($tgl=""){
		...
		$date=$tgl;	
		//Current Day PSB
		$current_query=$this->db2->query("SELECT count(psb_id) as amount FROM new_psb 
		WHERE status='sukses' AND DATE_FORMAT(tanggal_aktif,'%Y-%m-%d')='".$date."'");	
		$current_result=$current_query->row();

		//Current Month PSB
		$curmonth_query=$this->db2->query("SELECT count(psb_id) as amount FROM new_psb 
	...
	public function dayByDaySales($tgl=""){
		...
		$sukses_query=$this->db2->query("SELECT DATE_FORMAT(tanggal_aktif, '%Y-%m-%d') as tanggal_aktif, count(psb_id) as amount, MONTHNAME('".$date."') as m_name FROM new_psb where
		...
		// $pending_query=$this->db2->query("SELECT DATE_FORMAT(tanggal_validasi, '%Y-%m-%d') as tanggal_validasi ,count(psb_id) as amount,MONTHNAME('".$date."') as m_name FROM new_psb where
	...
	public function getTopBranch($limit='',$tgl='')
	{

		$sukses_query=$this->db2->query("SELECT b.nama_branch, MONTHNAME('".$tgl."') as m_name, count(a.psb_id) as amount 
			FROM new_psb a 
	...
	public function getTopPaket($limit='',$tgl='')
	{

		$paket_query=$this->db2->query("SELECT b.nama_paket, MONTHNAME('".$tgl."') as m_name, count(a.psb_id) as amount 
			FROM new_psb a 
	...
	public function getTopChannel($limit='',$tgl='')
	{

		$channel_query=$this->db2->query("SELECT sales_channel, MONTHNAME('".$tgl."') as m_name, count(psb_id) as amount 
			FROM new_psb 
	...
	public function getTopTL($limit='',$tgl='')
	{

		$tl_query=$this->db2->query("SELECT b.nama, c.nama_branch, count(a.psb_id) as amount 
			FROM new_psb a
	...
	public function dayByDaySalesCari($tgl){
		$sukses=array();		
		$pending=array();
	...
	public function getContrBranch($tgl = ''){
		$date=$tgl;	
		$query=$this->db2->query("SELECT b.nama_branch, count(a.psb_id) as jumlah 
			FROM new_psb a
			JOIN branch b ON a.branch_id=b.branch_id
			WHERE a.status='sukses' AND DATE_FORMAT(a.tanggal_aktif,'%Y-%m-%d') between 
