...
$(document).ready(function(){
  ...
  $('#add-bast').on('submit',function(){    
    ...
    if(msisdn.substring(0,3)!=628){
      ...
      $.ajax({
        method : "POST",
        url : "<?php echo site_url('Admin/bast_validasi').'/'.$no_bast.'/'.$psb_id.'/insert' ?>",
        data : $(this).serialize(),
        beforeSend : function(){
        ...
        if(data=="true"){  
          ...
          setTimeout(function(){ document.location.href = '<?php echo base_url()?>Admin/bast_validasi/'+no_bast+'/'+psb_id; }, 2000);
        }else{
          failedAlert2(data);
          $(".block-ui").css('display','none');
        }   
